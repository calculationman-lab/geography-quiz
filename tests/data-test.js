...
for(const q of questions){
  ...
  assert.strictEqual(q.choices.length,4,`${q.id}: 4択`);
  assert.strictEqual(new Set(q.choices).size,4,`${q.id}: 選択肢重複なし`);
  assert(Number.isInteger(q.answerIndex)&&q.answerIndex>=0&&q.answerIndex<4,`${q.id}: 正答位置`);
...
console.log("PASS: 589問（夏期139・前期450）、教材・単元タグ、4択、正答を検証");
