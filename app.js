(function(){
  ...
  function renderHome(){
    ...
    $("question-total").textContent=QUESTIONS.length;$("today-status").classList.toggle("done",Boolean(today));
  ...
  function startMain(){state.mode="main";state.deck=shuffle(QUESTIONS).slice(0,Math.min(SET_SIZE,QUESTIONS.length)).map(makeRound);state.index=0;state.score=0;state.answers=[];state.startedAt=Date.now();state.mainResult=null;showScreen("quiz");renderQuestion()}
  ...
  function answer(selectedIndex){
    const q=state.deck[state.index];const correct=selectedIndex===q.correctIndex;const buttons=[...$("choices").children];buttons.forEach((button,index)=>{button.disabled=true;if(index===q.correctIndex)button.classList.add("correct");if(index===selectedIndex&&!correct)button.classList.add("wrong")});if(correct)state.score++;
  ...
  $("start-button").addEventListener("click",startMain);$("next-button").addEventListener("click",next);$("review-button").addEventListener("click",startReview);$("home-button").addEventListener("click",goHome);$("quit-button").addEventListener("click",()=>{if(confirm("クイズを途中でやめてTOPへ戻りますか？"))goHome()});$("clear-history-button").addEventListener("click",()=>{if(confirm("これまでの記録をすべて消しますか？")){saveProgress({version:1,history:[]});renderHome()}});
  validateQuestions();renderHome();
})();
